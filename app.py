...
def load_data_from_github(github_url=None):
    """Robust data loader with error handling"""
    try:
...
def create_sample_data():
    ...
    line_types = ['Off', 'Middle', 'Leg', 'Wide', 'Down']
    ...
    data = {
        ...
        'battingConnectionId': np.random.choice(connection_types, 1000, 
        ...
        'commentary': np.random.choice(
            ['Good shot!', 'Perfect timing!', 'Mistimed!', 
             'Great connection!'] * 250 + 
             ['Boundary!', 'Six!', 'Edge!', 'Body hit!'] * 100,
            1000
        ),
    ...
    # Realistic boundary assignments
    ...
    df['commentary'] = df.apply(
        lambda x: 
            f"SIX! {x['battingShotTypeId']}" if x['runs'] == 6 
            else f"FOUR! {x['battingShotTypeId']}" if x['runs'] == 4 
            else f"{x['battingShotTypeId']} shot for {x['runs']} runs",
        axis=1
    )
