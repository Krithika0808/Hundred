...
def main():
    ...
    with tab1:
        ...
        if selected_player:
            ...
            theta = np.radians(player_data['shotAngle'])
            r = player_data['shotMagnitude']
            
            # Color by control score
            colors = ['red' if score < 50 else 'orange' if score < 80 else 'green' for score in player_data['control_score']]
            sizes = player_data['runs'] * 20 + 10
            
            scatter = ax.scatter(theta, r, c=colors, s=sizes, alpha=0.7)
            ...
            ax.set_title(f'{selected_player} - Shot Placement', pad=20)
            
            # Add fielding position labels
            angles = np.array([0, 45, 90, 135, 180, 225, 270, 315])
            labels = ['Long Off', 'Cover', 'Point', 'Third Man', 'Fine Leg', 'Square Leg', 'Mid Wicket', 'Long On']
            ax.set_thetagrids(np.radians(angles), labels)
    ...
    with tab2:
        ...
        scatter = ax.scatter(
            shot_analysis['is_controlled_shot'], 
            shot_analysis['runs'],
            s=shot_analysis['shot_count'] * 10,
            alpha=0.7
        )
        
        # Add labels for each point
        for i, row in shot_analysis.iterrows():
            ax.annotate(row['battingShotTypeId'], 
                       (row['is_controlled_shot'], row['runs']),
                       xytext=(5, 5), textcoords='offset points', fontsize=8)
        
        ax.set_xlabel('Control Rate')
        ax.set_ylabel('Average Runs')
        ax.set_title('Control vs Aggression Matrix')
    ...
    with tab3:
        ...
        for player in selected_players:
            ...
            with col1:
                favorite_shot = player_data['battingShotTypeId'].value_counts().index[0]
                st.markdown(f"**Favorite Shot**: {favorite_shot}")
                
                dismissal_zone = player_data[player_data['control_score'] < 50]['shotAngle'].median()
                st.markdown(f"**Dismissal Zone**: {dismissal_zone:.0f}°")
            
            with col2:
                strength_zone = player_data[player_data['control_score'] > 80]['shotAngle'].median()
                st.markdown(f"**Strength Zone**: {strength_zone:.0f}°")
                
                effective_shot = player_data.groupby('battingShotTypeId')['runs'].mean().idxmax()
                st.markdown(f"**Most Effective**: {effective_shot}")
